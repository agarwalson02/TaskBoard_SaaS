...
def convert_to_https_request(fastapi_request: Request)->httpx.Request:
...
def get_current_user(request: Request)->AuthUser:
    httpx_request= convert_to_https_request(request)
    ...
    if not org_id:
        raise HTTPException(
            ...
            detail="No organisation selected"
        )
...
def require_create(user: AuthUser=Depends(get_current_user))->AuthUser:
    if not user.can_create:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Create permission denied"
        )
    return user
