...
from app.core.auth import get_current_user, require_view, require_edit, require_delete, require_create
from app.models.tasks import Task
from app.schemas.tasks import TaskCreate, TaskUpdate, TaskStatusUpdate,TaskResponse
...
def update_task(task_id:str,task_data:TaskUpdate,user:AuthUser=Depends(require_edit),db:Session=Depends(get_db)):
    task=db.query(Task).filter(Task.id==task_id, Task.org_id==user.org_id).first()
    if not task:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,detail="Task not found")
